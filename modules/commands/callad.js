...
module.exports.handleReply = async function({ api, args, event, handleReply, Users}) {
 ...
 switch(handleReply.type) {
   ...
   break;}
    case "calladmin": {
      api.sendMessage({ body: `🌃 Phản hồi từ Bhan hoặc VVP [ ${name} ] da gui đến bạn: \n ________________\n➢  ${event.body} \n________________\n⚔ Reply tin nhắn này để tiếp tục gửi tin nhắn về cho Bhan và VVP🐧`, mentions: [{tag: name, id : event.senderID}]}, handleReply.id, (e, data) => global.client.handleReply.push({
  name: this.config.name,
  author: event.senderID,
  messageID: data.messageID,
